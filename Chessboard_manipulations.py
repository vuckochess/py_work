...
def retrieve_coordinates(coord_matrix, left, bottom, dim):
    ...
    coord_matrix[left+dim//2, bottom] = \
        line_a.intersection(line_med).to_array()
    coord_matrix[left+dim//2, bottom+dim] = \
        line_h.intersection(line_med).to_array()

    line_upright = line_a.medium_slope(line_h, intercept)
    coord_matrix[left, bottom+dim//2] = \
        line_8.intersection(line_upright).to_array()
    coord_matrix[left+dim, bottom+dim//2] = \
        line_1.intersection(line_upright).to_array()
...
class Line:
    ...
    def intersection(self, line):
        if self.slope == line.slope:
            print("Lines are parallel:", self.slope,
                  self.y_intercept, ";", line.slope, line.y_intercept)
            return None
...
def center_coordinates(coord_matrix):
    ...
    for i in range(row_dim-1):
        for j in range(column_dim-1):
            center_matrix[i, j] = square_center(
                coord_matrix[i, j],
                coord_matrix[i, j+1],
                coord_matrix[i+1, j+1],
                coord_matrix[i+1, j])
            scale_matrix[i, j] = (
                (coord_matrix[i+1, j][0]
                 - coord_matrix[i+1, j+1][0])
                / (coord_matrix[row_dim-1, j][0]
                   - coord_matrix[row_dim-1, j+1][0]))
...
def random_rotate_and_translate(image, coord, max_rot=5, max_transl=(30, 20)):
    ...
    translate = [random_symmetric(max_transl[0]),
                 random_symmetric(max_transl[1])]
...
def make_lower_resolution(image, scale):
    ...
    image = image.resize(
        (round(width/scale), round(height/scale))).resize((width, height))
    return image
...
class ImageScaling:
    ...
    def __init__(self, image, coords):
        ...
        self.center_spots, self.scale_matrix = \
            center_coordinates(self.coord_matrix)
        self.basis = np.sqrt(
            np.square(self.center_spots[7, 0, 0]
                      - self.center_spots[7, 1, 0]) +
            np.square(self.center_spots[7, 0, 1]
                      - self.center_spots[7, 1, 1]))
    ...
    def extract_image(self, row, column, train):
        ...
        angle = (-np.degrees(np.arctan(x_diff/y_diff))
                 + random_symmetric(i_scale.ANGLE_CHANGE[option]))
        translate = [-np.tan(np.radians(angle))*local_scale/2
                     + random_symmetric(i_scale.COORD_CHANGE[option]),
                     random_symmetric(i_scale.COORD_CHANGE[option])]
        croped_image = croped_image.rotate(angle, translate=translate)
        width = (box[2]-box[0]) / 4,
        height = (box[3]-box[1]) / (2*(i_scale.UP_SCALE_RATIO+1))
        small_box = np.array([
            width,
            height*i_scale.UP_SCALE_RATIO,
            width*3,
            height * (2*i_scale.UP_SCALE_RATIO + 1)]
            ).round()
